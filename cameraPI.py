...
'''
Inicie esto junto con el módulo de la cámara para guardar fotos en una carpeta con un nombre de archivo, por ejemplo, imagen-<contador>.jpg
* agrega '_' después de una palabra, el siguiente número y el formato del archivo
'''
...
#         return (self.new_filename(requested_filename) + suffix)
    ...
    def new_filename(self, requested_filename):
        ...
        if requested_filename == None and self.last_request_filename == None:
            raise Exception('Ingrese un nombre de archivo para el primer uso de la funcion')
...
# Funciones que puede llamar el usuario
## Funciones principales
## Funciones de configuración
# Funciones internas
## Funciones internas de alto nivel
## Nivel bajo

##################### FUNCIONES que se pueden llamar #####################

########### FUNCIONES PRINCIPALES DE FOTOGRAFÍA ###########
    def __init__(self, spi_bus, cs, skip_sleep=False, debug_information=False):
        self.cs = cs
        self.spi_bus = spi_bus
        self.DEBUG_MODE = debug_information

        self._write_reg(self.CAM_REG_SENSOR_RESET, self.CAM_SENSOR_RESET_ENABLE) #Reiniciar la cámara
        self._wait_idle()
        self._get_sensor_config() # Obtener información del sensor de la cámara
        self._wait_idle()
        ...
        # Establecer el formato y la resolución predeterminados
        self.current_pixel_format = self.CAM_IMAGE_PIX_FMT_JPG
        self.old_pixel_format = self.current_pixel_format
        
        self.current_resolution_setting = self.RESOLUTION_640X480 # El controlador ArduCam define esto como modo
        ...
        # Parece que esto sobrescribe el valor, así que lo estoy eliminando. ¿Configure esto antes de self._get_sensor_config()?
        #self.camera_idx = 'NO DETECTADO'

        # Realiza un seguimiento del tiempo de calentamiento de AWB
    ...
    def startup_routine_3MP(self):
        ...
        if self.DEBUG_MODE: print('Ejecutando la rutina de inicio de 3MP')
        self.capture_jpg()
        self.saveJPG('imagen ficticia.jpg')
        uos.remove('imagen ficticia.jpg')
        if self.DEBUG_MODE: print('Rutina de inicio de 3MP finalizada')
    '''
    ######

    '''
    Emite una advertencia si la ruta del archivo no termina en .jpg (en blanco) y agrega

    Emite un error si el tipo de archivo NO es .jpg
    '''
    def capture_jpg(self):
        ...
        if (utime.ticks_diff(utime.ticks_ms(), self.start_time) <= self.WHITE_BALANCE_WAIT_TIME_MS) and self.camera_idx == '5MP':
            print('Agrega un', self.WHITE_BALANCE_WAIT_TIME_MS, 'ms delay para permitir que se ejecute el balance de blancos')
        else:

            if self.DEBUG_MODE: print('Captura ingresada_jpg')
            ...
            self._set_capture()
            if self.DEBUG_MODE: print('Captura__jpg Finalizada')
    ...
    def saveJPG(self,filename):
        headflag = 0
        print('Guardando la imagen, no desconecte la alimentación')
    ...
    def resolution(self, new_resolution):
        ...
        if self.camera_idx == '3MP':
            if input_string_lower in self.valid_3mp_resolutions:
                ...
                raise ValueError("Resolución no válida proporcionada para {}, seleccione entre {}".format(self.camera_idx, list(self.valid_3mp_resolutions.keys())))

        elif self.camera_idx == '5MP':
            if input_string_lower in self.valid_5mp_resolutions:
                self.current_resolution_setting = self.valid_5mp_resolutions[input_string_lower]
            else:
                raise ValueError("Resolución no válida proporcionada para {}, seleccione entre {}".format(self.camera_idx, list(self.valid_5mp_resolutions.keys())))
...
#     CONTRAST_PLUS_3 = 5
    ...
    def set_white_balance(self, environment):
        ...
        if environment == 'sunny':
            ...
            print('ACTUALIZACIÓN POR HACER: Para obtener mejores resultados, configure una configuración de balance de blancos')
...
########### CORE PHOTO FUNCTIONS ###########
    ...
    def _set_capture(self):
        if self.DEBUG_MODE: print('Se ingreso _set_capture')
        self._clear_fifo_flag()
        self._wait_idle()
        self._start_capture()
        if self.DEBUG_MODE: print('bandera fifo borrada, _start_capture iniciado, esperando CAP_DONE_MASK')
        while (int(self._get_bit(self.ARDUCHIP_TRIG, self.CAP_DONE_MASK)) == 0):
            if self.DEBUG_MODE: print("Registro ARDUCHIP_TRIG, CAP_DONE_MASK:", self._get_bit(self.ARDUCHIP_TRIG, self.CAP_DONE_MASK))
            sleep_ms(100)
        if self.DEBUG_MODE:print('finalizó la espera de _start_capture')
        ...
        self.received_length = 320 * 240 * 2  # Asumir un tamaño fijo para la imagen
        self.total_length = self.received_length
        self.burst_first_flag = False
        if self.DEBUG_MODE: print('se leyó la longitud fifo'
)
    
    def _read_fifo_length(self): # TODO: CONFIRM AND SWAP TO A 3 BYTE READ
        if self.DEBUG_MODE: print('Se ingresó _read_fifo_length')
        ...
        if self.DEBUG_MODE: 
            print("bytes de longitud fifo (int):")
            print(len1,len2,len3)
        if ((((len3 << 16) | (len2 << 8) | len1) & 0xffffff)>5000000): 
            print("Error La longitud fifo es demasiado larga >5 MB Tamaño: ", (((len3 << 16) | (len2 << 8) | len1) & 0xffffff))
            print("Arducam posiblemente no tomó una fotografía y está devolviendo datos basura")
