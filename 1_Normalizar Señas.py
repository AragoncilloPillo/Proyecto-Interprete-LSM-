    ...
    for filename in sorted(os.listdir(directory)):
        if filename.endswith('.jpg'):
            ...
            if frame is not None:
                ...
                print(f"Alerta: no se puede leer el marco {filename}")
    return frames

def interpolate_frames(frames, target_frame_count=23):
    ...
    for i in indices:
        ...
        weight = i - lower_idx
        if upper_idx < current_frame_count:
            interpolated_frame = cv2.addWeighted(frames[lower_idx], 1 - weight, frames[upper_idx], weight, 0)
        else:
            interpolated_frame = frames[lower_idx] # Manejar el caso donde Upper_idx está fuera de rango
        interpolated_frames.append(interpolated_frame)
    return interpolated_frames

def normalize_frames(frames, target_frame_count=23):
    current_frame_count = len(frames)
    if current_frame_count < target_frame_count:
...
def process_directory(word_directory, target_frame_count=23):
    for sample_name in os.listdir(word_directory):
        ...
        if os.path.isdir(sample_directory):
            ...
            if frames:
                ...
                print(f"No se encontraron marcos en {sample_directory}")

def save_normalized_frames(directory, frames):
    for i, frame in enumerate(frames, start=1):
        cv2.imwrite(os.path.join(directory, f'Toma_{i:02}.jpg'), frame, [cv2.IMWRITE_JPEG_QUALITY, 80])
...
if __name__ == "__main__":
    ...
    for word_id in word_ids:
        ...
        if os.path.isdir(word_path):
            ...
            print(f"{word_path} no es un directorio válido")
